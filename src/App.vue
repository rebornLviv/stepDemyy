    ...
    <v-app-bar app color="white" dark class="barrr">
        ...
        <v-menu bottom origin="center center" transition="scale-transition" :absolute="absolute">
            ...
            <v-list>
                    ...
                    <v-list-item-title>
                        <v-row>
                            <v-dialog v-model="dialog3" max-width="600px">
                                <template v-if="!isUserLoggedIn" v-slot:activator="{ on }">
                                    <v-btn  text class="black--text" color="primary" dark v-on="on" @click.stop="dialog3">Зареєструватись</v-btn>
                                </template>
                ...
                <v-btn v-if="isUserLoggedIn" to="/profile" text class="black--text">Особистий Кабінет</v-btn>
